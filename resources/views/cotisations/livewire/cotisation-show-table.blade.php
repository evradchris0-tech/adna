<div>
    <div class="body_header d_flex jc_between ai_center">
        <h1 class="body_header_title">Details cotisations</h1>
            ...
            <table>
                ...
                <tbody>
                    @foreach ($cotisations as $cotisation)
                        <tr>
                            <td><span>{{ $cotisation->id }}</span></td>
                            <td><span>{{ $cotisation->type }}</span></td>
                            <td><span>{{ formatNumber($cotisation->somme) }} XAF</span></td>
                            <td><span>{{ $cotisation->created_at }}</span></td>
                            <td>
                                <div class="t_action">
                                    @can('cotisations.update')
                                        <a href="{{ route('cotisations.update', $cotisation->id) }}"
                                            class="update btn_action"> <i class="fa fa-pencil"></i> </a>
                                    @endcan
                                    @can('cotisations.delete')
                                        <div class="update btn_action"
                                            wire:click='onSHowDeleteModal({{ $cotisation->id }})'> <i
