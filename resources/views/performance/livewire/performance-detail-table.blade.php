<div>
    <div class="body_header d_flex jc_between ai_center">
        <h1 class="body_header_title">Performance ({{$association->name}})</h1>
    ...
    <div class="card">
        ...
        <div class="card_header filters">
            ...
            <div class="form_group">
                <label for="study_level">Situation</label>
                <div class="form_input">
                    <select name="study_level" id="study_level" wire:model="situation">
        ...
        <div class="card_content">
            <table class="max-content">
                <thead>
                    <tr>
                    <x-table-th :direction="$orderDirection" label="Ancien matricule" name="old_matricule" :field="$orderField">Ancien matricule</x-table-th>
                    <x-table-th :direction="$orderDirection" label="Nom & Prenom" name="name" :field="$orderField">Nom & Prenom</x-table-th>
                    <th>Dime</th>
                    {{-- <th>Offrande recolte</th> --}}
                    <th>Offrande construction</th>
                ...
                <tbody>
                    @foreach ($performances as $perf)
                        <tr>
                            <td><span>{{$perf->old_matricule}}</span></td>
                            <td><span>{{$perf->name}}</span></td>
                            <td><span>{{formatNumber($perf->performance["dimeR"])}} / {{formatNumber($perf->performance["dime"])}} FCFA</span></td>
                            {{-- <td><span>{{$perf->performance["offrandeR"]}} / {{$perf->performance["offrande"]}} FCFA</span></td> --}}
                            <td><span>{{formatNumber($perf->performance["cotisationR"])}} / {{formatNumber($perf->performance["cotisation"])}} FCFA</span></td>
